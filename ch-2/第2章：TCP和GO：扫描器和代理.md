- [摘要](#摘要)
- [理解TCP握手](#理解TCP握手)
- [通过端口转发绕过防火墙](#通过端口转发绕过防火墙)
- [编写TCP扫描程序](#编写TCP扫描程序)
    - [测试端口可用性](#测试端口可用性)
    - [非并发的扫描](#非并发的扫描)
    - [并发扫描](#并发扫描)
        - [“更快的”扫描器版本](#“更快的”扫描器版本)
        - [使用WaitGroup同步扫描](#使用WaitGroup同步扫描)
        - [多管道通讯](#多管道通讯)
- [构建TCP代理](#构建TCP代理)
    - [使用io.Reader和io.Writer](#使用io.Reader和io.Writer)
    - [创建Echo服务](#创建Echo服务)
    - [通过创建带有缓冲的侦听器来改进代码](#通过创建带有缓冲的侦听器来改进代码)
    - [代理TCP客户端](#代理TCP客户端)
    - [复制Netcat执行命令](#复制Netcat执行命令)
- [总结](#总结)
...
<div align=center><img width = '420' height ='500' src ="https://github.com/YYRise/black-hat-go/raw/dev/ch-2/images/2-1.png"/></div>
<center> 图2-1：TCP握手原理 </center>
...
<div align=center><img width = '500' height ='85' src ="https://github.com/YYRise/black-hat-go/raw/dev/ch-2/images/2-2.png"/></div>
<center> 图2-2：TCP代理 </center>
...
注意，对 reader.Read([]byte) 和 writer.Write([] byte) 的显式调用已替换为对 io.Copy(writer, reader) 的调用。在函数内部，io.Copy(writer, reader) 调用 reader 的 Read([]byte) 函数时, 将会触发FooReader从stdin读取。
...
func main() {
  ...
  ❼ go echo(conn)
```
代码 2-11: 基本的echo服务 (https://gihub.com/blackhat-go/bhg/ch-2/echo-server/main.go/)
...
首先，调整代码以显式地强制刷新 stdout 来纠正此细微差别。
